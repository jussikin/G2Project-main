 ...
 *      G2 $Id: smtp.php 20940 2009-11-20 19:56:16Z mindless $
 *
 ***************************************************************************/
...
function server_parse(&$socket, $response)
{
    for ($server_response = ''; substr($server_response, 3, 1) != ' ';) {
        if (!($server_response = fgets($socket, 256))) {
            return GalleryCoreApi::error(ERROR_PLATFORM_FAILURE, __FILE__, __LINE__, "Couldn't get mail server response code");
        }
    }
    if (!(substr($server_response, 0, 3) == $response)) {
        return GalleryCoreApi::error(ERROR_PLATFORM_FAILURE, __FILE__, __LINE__, "Problem sending mail. Server response: $server_response");
    }
...
function smtpmail($config, $to, $subject, $body, $headers = null)
{
    ...
    /* Connect */
    list($config['smtp.host'], $port) = array_merge(explode(':', $config['smtp.host']), array(
        25
    ));
