...
function app_init()
{
    if (!app_is_ie && !app_is_safari || app_is_ie7) {
        album_setfixedtitle();
    }
    app_body = document.getElementById('hybridMain');
    while (app_body && app_body.tagName != 'BODY') {
        app_body = app_body.parentNode;
    }
    if (app_is_ie) {
        app_body = app_body.parentNode;
    }
  ...
  // visible, Firefox only when object is invisible.  Easier to just use iframe.
    ...
    if (window.attachEvent) {
        window.attachEvent("onresize", app_onresize);
    } else if (window.addEventListener) {
        window.addEventListener("resize", app_onresize, false);
    }
...
function app_getcookie()
{
    ...
    if (c) {
        ...
        if (c == 2 /*start slideshow*/) {
            slide_onoff(slide_order < 0 ? data_view == 0 : data_view > 0);
        } else if (data_view >= 0) {
            image_show(data_view);
        }
    }
}
            ...
            case 34: //PageDown
                if (text_on) {
                    ...
                    obj.scrollTop += (keyCode==34 ? 1 : -1) * (obj.clientHeight - 5);
                      ...
                      break;
            case 238: //Ctrl-Up
                if (document.getElementById('full_size').style.display == 'inline') {
                    image_zoom(1);
                }
                      break;
            case 240: //Ctrl-Down
                if (document.getElementById('fit_size').style.display == 'inline') {
                    image_zoom(0);
                }
                      break;
        }
...
{
    if (on) {
        app_body.saveScrollTop = app_body.parentNode.scrollTop; } //For gecko
    app_body.style.overflow = on ? 'hidden' : 'auto';
    if (!on) {
        app_body.parentNode.scrollTop = app_body.saveScrollTop; } //For gecko
    app_getwinsize();
    if (!on && slide_on) {
        slide_onoff();
    }
    if (app_is_ie && sidebar_on) {
        ui_vis('sidebar', on?0:1); //For IE (hide <select>s)
    }
    ui_vis('imageview', image_on=on);
    if (!on) {
        if (!app_is_safari) {
            imagearea.scrollLeft = imagearea.scrollTop = 0; //avoid Safari crash
        }
        imagediv.innerHTML = '';
    } else if (app_is_ie) {
...
function image_show(i)
{
    ...
    image_setbuttons();
    text_fill();
    if (slide_inprog && !slide_on) {
        slide_inprog = 0;
    }
}
function image_fit(getstr)
{
    if (!getstr && !document.view) {
        return;
    }
...
function image_zoom(on,noresize)
{
    ...
    if (!on) {
        imagearea.scrollLeft = imagearea.scrollTop = 0;
    }
    if (image_zoomon=on) {
        ui_vis('full_size', 0);
        ui_vis('fit_size', 1, 1);
...
{
    var i = slide_nextindex(), j = i; if (j < 0 || image_iscached[j]) {
        j = slide_previndex();
    }
    if (j >= 0) {
        image_precache(j);
...
function image_next()
{
    var i = slide_nextindex(); if (i >= 0) {
        image_show(i);
    } else {
        var go = (slide_on && slide_order < 0) ? prev_page : next_page;
        if (go) {
            location.href = go.replace('__SLIDE__', slide_on); } else {
            return -1;
            } }
}
function image_prev()
{
    var i = slide_previndex(); if (i >= 0) {
        image_show(i);
    } else {
        var go = (slide_on && slide_order < 0) ? next_page : prev_page;
        if (go) {
            location.href = go.replace('__SLIDE__', slide_on);
        } }
}
...
function text_onoff()
{
    if ((text_on = text_on?0:1) && data_count > 0) {
        text_fill();
    }
...
{
    if (text_on) {
        ui_sethtml('text', document.getElementById('text_'+image_index).innerHTML);
    }
}
...
function slide_nextindex(x)
{
    if (slide_on && !slide_order) {
        for (var i=0, j=0; i < data_count; i++) {
            if (slide_randomorder[i] == image_index) {
                j = i + (x?-1:1); break; }
        }
...
function slide_setdelay(d)
{
    ...
    if (slide_on) {
        slide_reset(); slide_go(slide_nextindex()); }
}
function slide_fillrandom(lockfirst)
{
    var i, j, k;
    for (i = 0; i < data_count; i++) {
        slide_randomorder[i] = i;
    }
...
function slide_onoff(start_last)
{
    ...
    if (slide_on) {
        ...
        if (slide_inprog) {
            ...
            if (!slide_order) {
                slide_fillrandom(image_on);
            }
            if (!image_on || slide_nextindex() < 0) {
                d = 5;
                t = 'image_show(';
                if (slide_order) {
                    t += (slide_order + (start_last?2:0) == 1 ? 0 : data_count - 1) + ')';
                } else {
                    t += (start_last ? slide_randomorder[data_count-1] : slide_randomorder[0]) + ')';
                }
            }
        }
        slide_inprog = 1;
