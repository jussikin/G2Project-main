...
function getCaches()
{
	$dirs = array(
		...
		'log' => array(
			...
			array(),
			'Install/Upgrade log files <span class="subtext important">' . '(can\'t be recovered!)</span>'
		),
		'derivative' => array(
			false,
			'clearG2dataDir',
...
function recursiveRmdir($dirname, &$status)
{
		...
		if (is_dir($path)) {
			...
			if (!@unlink($path)) {
				if (!@is_writeable($path)) {
					$status[] = array(
						'error',
						"Permission denied removing file $path"
					);
				} else {
					$status[] = array(
						'error',
	...
	if (!@rmdir($dirname)) {
		$status[] = array(
			'error',
			"Unable to remove directory $dirname"
		);
	} else {
		$count++;
	}
...
function clearInstallUpgradeLogs()
{
	...
	if ($fd = opendir($path)) {
		while (($filename = readdir($fd)) !== false) {
			if (preg_match('/^(install|upgrade)_[0-9a-f]+\.log$/', $filename) && is_file($path . $filename)) {
				if (@unlink($path . $filename)) {
					$count++;
				} else {
					$status[] = array(
						'error',
						"Error removing $path$filename"
					);
				}
			}
		}
	...
	$status[] = array(
		'info',
		"Removed $count install/upgrade log files"
	);
