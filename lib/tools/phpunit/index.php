...
/**
 * Load up index.php so that tests that want _GalleryMain can get to it.  Do it now, though so that
...
function PhpUnitOutputInterceptor($message)
{
    ...
    if (isset($_GET['filter'])) {
        ...
        if (empty($replaced) && strstr($message, '<!--base_href-->')) {
            $replaced = true;
            fwrite($fd, str_replace('<!--base_href-->', @sprintf('<base href="http://%s:%d/%s/">', $_SERVER['HTTP_HOST'], $_SERVER['SERVER_PORT'], dirname($_SERVER['SCRIPT_NAME'])), $message));
        } else {
            fwrite($fd, $message);
        }
    }
...
function PhpUnitGalleryMain(&$testSuite, $filter)
{
    ...
    if ($isSiteAdmin && $filter !== false) {
        ...
        $suiteArray += loadTests('repositorytools', dirname(__FILE__) . '/../repository/test/phpunit', $filter);
    ...
    if ($platform->file_exists($testDir) && $platform->is_dir($testDir) && $dir = $platform->opendir($testDir)) {
        ...
        while (($file = $platform->readdir($dir)) != false) {
            if (preg_match('/(.*Test).class$/', $file, $matches)) {
                ...
                $className = $matches[1];
                if (class_exists($className) && GalleryUtilities::isA(new $className(null), 'GalleryTestCase')) {
...
class GalleryTestResult extends TestResult
{
    ...
    public function report()
    {
        ...
        if (!isset($compactView)) {
            ...
            if ($this->_testsRunThenSkipped) {
                printf("setTxt('runThenSkip',' (of those, %s skipped)');", $this->_testsRunThenSkipped);
            }
            printf("setTxt('testFailCount','%s test%s');", $this->_testsFailed, ($this->_testsFailed == 1) ? '' : 's');
            printf("setTxt('testErrorCount','%s error%s');", $nFailures, ($nFailures == 1) ? '' : 's');
    ...
    public function _getTestResultRecord()
    {
        ...
        $buf = sprintf('|%s||%s||%s||%s||%s||%s||%s||%d||%d||%s||%s', $webserver, $php, $database, $OS, $locking, $language, $owner, $count, $failed, $date, join(' ', $notes));
    ...
    public function _endTest($test)
    {
        ...
        if ($test->wasSkipped()) {
            ...
            if ($test->failed()) {
                ...
                $failure .= '<li><a href="?filter=' . urlencode('^' . $test->getModuleId() . '.' . $test->classname() . '.' . $test->name() . '$') . '" name="fail' . $this->fRunTests . '">' . $test->classname() . '.' . $test->name() . "</a></li><ul>\n";
                ...
                if (isset($testOneByOne)) {
                    ...
                    echo '<meta http-equiv="refresh" content="0; index.php?filter=' . "$x$i-$i" . '&amp;onebyone=true"/>';
                }
                $cmd = "updateStats(1, 0, 0, $usedMemory)";
            }
        }
        echo '<script type="text/javascript">r=document.getElementById(\'testRow' . $this->fRunTests . "');$extra";
...
if (isset($_GET['filter'])) {
    ...
    foreach ($skip as $tempSkip) {
        if (preg_match('/(\d+)-(\d+)/', $tempSkip, $matches)) {
            if ($matches[1] >= 1 && $matches[1] <= FILTER_MAX && $matches[2] >= 1 && $matches[2] <= FILTER_MAX) {
                $range[] = array(
                    $matches[1],
                    $matches[2]
                );
