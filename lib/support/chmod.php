...
if (empty($status['error'])) {
	...
	switch ($command) {
		case CMD_ADVANCED:
			...
			$ret = chmodRecursively($path, $folderPermissions->getAsInt(), $filePermissions->getAsInt(), time() - 60);
			if (!empty($ret)) {
				$status['error'][] = "Failed to change the filesystem permissions " . "of '$path'.";
			} else {
				$status['message'] = "Successfully changed the filesystem permissions " . "of '$path'.";
			}
			break;
		case CMD_CHMOD_MODULES_AND_THEMES_DIR:
			...
			if (!in_array($mode, array(
				...
				if (!empty($ret)) {
					$status['error'][] = 'Failed to change the filesystem permissions ' . 'of the modules/ and themes/ folder.';
				} else {
					$status['message'] = 'Successfully changed the filesystem permissions ' . 'of the modules/ and the themes/ folder.';
				}
			}
			break;
		case CMD_CHMOD_PLUGIN_DIR:
			...
			if (!isset($plugins[$pluginPath])) {
				...
				if (!empty($ret)) {
					$status['error'][] = "Failed to change the filesystem permissions " . "of the '$pluginPath' folder.";
				} else {
					$status['message'] = "Successfully changed the filesystem permissions " . "of the '$pluginPath' folder.";
				}
			}
		...
		case CMD_CHMOD_GALLERY_DIR:
			...
			if (!in_array($mode, array(
				...
				if (!empty($ret)) {
					$status['error'][] = 'Failed to change the filesystem permissions ' . 'of the Gallery folder.';
				} else {
					$status['message'] = 'Successfully changed the filesystem permissions ' . 'of the Gallery folder.';
				}
			}
			break;
		case CMD_CHMOD_STORAGE_DIR:
			...
			if (!empty($ret)) {
				$status['error'][] = 'Failed to change the filesystem permissions ' . 'of the storage folder.';
			} else {
				$status['message'] = 'Successfully changed the filesystem permissions ' . 'of the storage folder.';
			}
			break;
		case CMD_CHMOD_LOCALE_DIR:
			...
			if (!empty($ret)) {
				$status['error'][] = 'Failed to change the filesystem permissions ' . 'of the locale folder.';
			} else {
				$status['message'] = 'Successfully changed the filesystem permissions ' . 'of the locale folder.';
			}
			break;
		default:
...
function chmodGalleryDirRecursively($makeItWriteable) {
	...
	return chmodRecursively(GallerySetupUtilities::getConfigDir(), $folderMode, $fileMode, time() - 60);
}
...
function printStatus($status) {
	...
	if (!empty($status['message'])) {
		printf('<script type="text/javascript">printStatusMessage(\'%s\');</script>', str_replace(array(
			"\\",
			"'"
		), array(
			"\\\\",
...
<html lang="en">
  ...
  <body>
    ...
    Change Filesystem Permissions
      ...
      <h2>
    <a href="<?php generateUrl('index.php?chmod&amp;command=' . CMD_CHMOD_MODULES_AND_THEMES_DIR . '&amp;mode=open'); ?>">Make modules &amp; themes directories writeable</a>
      </h2>
      <p class="description">
    ...
    program or command line shell.
        ...
        <?php else : ?>
      <h2>
    <a href="<?php generateUrl('index.php?chmod&amp;command=' . CMD_CHMOD_MODULES_AND_THEMES_DIR . '&amp;mode=secure'); ?>">Make modules &amp; themes directories read-only</a>
      </h2>
      <p class="description">
    ...
    <p class="description">
      ...
      <h2><a href="<?php generateUrl('index.php?chmod&amp;command=' . CMD_CHMOD_STORAGE_DIR); ?>">Make the data folder read/write</a></h2>
      <p class="description">
      ...
      <h2><a href="<?php generateUrl('index.php?chmod&amp;command=' . CMD_CHMOD_LOCALE_DIR); ?>">Make the locale folder read/write</a></h2>
      <p class="description">
      ...
      <h2><a href="<?php generateUrl('index.php?chmod&amp;command=' . CMD_CHMOD_GALLERY_DIR); ?>&amp;mode=open">Make everything read/write</a></h2>
      <p class="description">
        ...
        <?php else : ?>
      <h2><a href="<?php generateUrl('index.php?chmod&amp;command=' . CMD_CHMOD_GALLERY_DIR); ?>&amp;mode=secure">Make everything read-only</a></h2>
      <p class="description">
    ...
    <p class="description">
      <b> Path to change: </b>
      <input type="text" name="path" size="50" value="<?php echo $path; ?>">
          <br>
      <span class="subtext">
        Gallery folder: <i><?php echo GallerySetupUtilities::getConfigDir(); ?></i> <br>
            Gallery data folder: <i><?php echo getGalleryStoragePath(); ?></i> <br>
      </span>
          <br>
          <b> New permissions: </b>
        <?php
	...
	$value = $permissionBitSet[0]->getAsString() . $permissionBitSet[1]->getAsString(); ?>
              ...
              <input id="set_<?php echo $value; ?>" type="radio" name="permissions" value="<?php echo $value; ?>" <?php echo $checked; ?>>
                <label for="set_<?php echo $value; ?>">
                  <span class="hasToolTip" title="Files: <?php echo $permissionBitSet[1]->getAsString(); ?>, Folders: <?php echo $permissionBitSet[0]->getAsString(); ?>"> 
                    <?php echo $permissionBitSet[1]->getDescription(); ?>
                  </span>
                </label>
              </input>
                <?php 																																																			endforeach; ?>
