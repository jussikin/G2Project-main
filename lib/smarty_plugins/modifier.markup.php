...
class GalleryBbcodeMarkupParser {
	...
	public function GalleryBbcodeMarkupParser() {
		...
		/* [b], [i] */
		$this->_bbcode->addCode('b', 'simple_replace', null, array(
			'start_tag' => '<b>',
			'end_tag' => '</b>'
		), 'inline', array(
			'listitem',
		...
		$this->_bbcode->addCode('i', 'simple_replace', null, array(
			'start_tag' => '<i>',
			'end_tag' => '</i>'
		), 'inline', array(
			'listitem',
		...
		$this->_bbcode->addCode('url', 'usecontent?', array(
			$this,
			'url'
		), array(
		...
		/* [color=...]Text[/color] */
		$this->_bbcode->addCode('color', 'callback_replace', array(
			$this,
			'color'
		), array(
			'usecontent_param' => 'default'
		...
		/* [img]http://...[/img] */
		$this->_bbcode->addCode('img', 'usecontent', array(
			$this,
			'image'
		), array(), 'image', array(
			'listitem',
		...
		/* [list] [*]Element [/list] */
		$this->_bbcode->addCode('list', 'simple_replace', null, array(
			'start_tag' => '<ul>',
			'end_tag' => '</ul>'
		), 'list', array(
			'block',
		...
		$this->_bbcode->addCode('*', 'simple_replace', null, array(
			'start_tag' => '<li>',
			'end_tag' => "</li>\n"
		), 'listitem', array(
			'list'
	...
	public function url($action, $attributes, $content, $params, &$node_object) {
		if ($action == 'validate') {
			...
			/* Output of HTML. The code is like [url]http://.../[/url] */
			if (!isset($attributes['default'])) {
	...
	public function image($action, $attrs, $content, $params, &$node_object) {
		if ($action == 'validate') {
			...
			/* Output of HTML. */
			$size = (isset($attrs['width']) ? ' width="' . (int) $attrs['width'] . '"' : '') . (isset($attrs['height']) ? ' height="' . (int) $attrs['height'] . '"' : '');
