...
if ($langpath == $path || !file_exists($langpath)) {
    if ($langpath != $path && !in_array(basename($langpath), array(
        'en.po',
        'zh.po'
    ))) {
...
function checkStringForBadUtf8($string, $path)
{
    if (!isValidUtf8($string)) {
        $printableString = preg_replace_callback('/([^\x20-\x7e])/', function ($m) {
            return "\\\\\\x" . dechex(ord($m[1]));
        }, $string);
        fwrite(stdErr(), "\nWarning: Translation contains invalid UTF-8" . " \"$printableString\" in file $path\n");
    }
}
...
function checkStringForHtml($string, $type, $path)
{
    ...
    if (preg_match($ltRegExpPattern, $string)) {
        fwrite(stdErr(), "\nWarning: Translation contains < (should be &lt;) in $type " . "\"$string\" in file $path\n");
    }
