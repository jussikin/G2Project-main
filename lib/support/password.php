		...
		if ($success == true) {
			...
			if (isset($_REQUEST['target'])) {
				...
				foreach ($_REQUEST['target'] as $key => $ignored) {
					...
					$args[0] = $caches[$key][2];
					$args[1] = $caches[$key][3];
					$status = array_merge($status, call_user_func_array($func, array(
						$args[0],
						$args[1]
					)));
					$remember[] = $key;
...
function recDelDir($dirname, &$status)
{
	...
	while (($filename = readdir($fd)) !== false) {
		...
		if (is_dir($path)) {
			...
			$status[] = array(
				'error',
				"Unable to remove cache file: $path"
			);
		}
	}
	closedir($fd);
	...
	if (!@rmdir($dirname)) {
		$status[] = array(
			'error',
			"Unable to remove cache directory: $dirname"
		);
	}
...
function refreshCache($dir, $mark)
{
	...
	if (@mkdir($path)) {
		$status[] = array(
			'info',
			"Refreshed $mark cache"
		);
	} else {
		$status[] = array(
			'error',
			"Unable to refresh $mark cache"
		);
	}
