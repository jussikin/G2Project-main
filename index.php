...
if ($gallery->isEmbedded()) {
	...
	if ($unsanitizedView == 'core.DownloadItem' && !empty($itemId)) {
	     ...
	     */
		if (isset($_SERVER['HTTP_IF_MODIFIED_SINCE']) || (function_exists('getallheaders') && ($headers = getallheaders()) && (isset($headers['If-Modified-Since']) || isset($headers['If-modified-since'])))) {
		...
		$path = GalleryDataCache::getCachePath(array(
			'type' => 'fast-download',
			'itemId' => $itemId
		));
...
function GalleryMain($embedded = false) {
	...
	if ($ret) {
		...
		echo '<h1> Debug Redirect </h1> ' . 'Not automatically redirecting you to the next page because we\'re in debug mode<br/>';
...
function _GalleryMain($embedded = false, $template = null) {
	...
	if (!empty($controllerName)) {
		...
		if ($gallery->getConfig('mode.maintenance') && !$controller->isAllowedInMaintenance()) {
			...
			if (!$isAdmin) {
				if (($redirectUrl = $gallery->getConfig('mode.maintenance')) === true) {
					$redirectUrl = $urlGenerator->generateUrl(array(
		...
		if (!isset($results['status']) || !isset($results['error']) || (!isset($results['redirect']) && !isset($results['delegate']) && !isset($results['return']))) {
			return array(
				GalleryCoreApi::error(ERROR_BAD_PARAMETER, __FILE__, __LINE__, 'Controller results are missing status, error, (redirect, delegate, return)'),
				null
			);
		}
		...
		if (empty($redirectUrl) && !empty($results['redirect'])) {
			...
			$redirectUrl = $urlGenerator->generateUrl($urlToGenerate, array(
				'forceFullUrl' => true
			));
		}
		...
		if ($view->isImmediate()) {
			...
			if (isset($results['redirect']) || isset($results['redirectUrl'])) {
				if (isset($results['redirectUrl'])) {
					...
					$redirectUrl = $urlGenerator->generateUrl($results['redirect'], array(
						'forceFullUrl' => true
					));
				}
			...
			if (empty($results['body'])) {
				return array(
					GalleryCoreApi::error(ERROR_BAD_PARAMETER, __FILE__, __LINE__, 'View results are missing body file'),
					null
				);
			}
...
function _GalleryMain_doRedirect($redirectUrl, $template = null, $controller = null, $ignoreErrors = false) {
	...
	if (!$session->isUsingCookies() && $session->isPersistent() && strpos($redirectUrl, $session->getKey()) === false) {
		$redirectUrl = GalleryUrlGenerator::appendParamsToUrl($redirectUrl, array(
