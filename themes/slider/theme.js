...
function app_onresize()
{
	...
	} else if (app_is_opera && !thumbs_horiz) {
		thumb_div.style.height = app_wh + 'px';
	}
	image_fit();
}
...
function app_onkeypress(event)
{
   ...
   */
	...
	switch (keyCode) {
		case 37: //Left
			if (image_zoomon) {
				image_area.scrollLeft -= 20; break; }
			case 137: //Shift-Left
				image_prev();
		  break;
		case 39: //Right
		...
		case 33: //PageUp
			if (thumbs_horiz) {
				thumb_div.scrollLeft -= app_ww - 10;
			} else {
				thumb_div.scrollTop -= app_wh - 10;
			}
		  break;
		case 34: //PageDown
			if (thumbs_horiz) {
		...
		case 36: //Home
			if (thumbs_horiz) {
				...
				thumb_div.scrollTop = 0;
			}
		  break;
		case 35: //End
			if (thumbs_horiz) {
				thumb_div.scrollLeft = thumb_div.scrollWidth;
			} else {
				thumb_div.scrollTop = thumb_div.scrollHeight;
			}
		  break;
	}
}
...
function image_setsize()
{
	...
	if (app_is_ie) {
		image_area.style.height = (app_wh - title_div.offsetHeight) + 'px';
	}
}
function image_show(i)
{
	...
	image_setsize();
	if (options_on) {
		options_setsize(); options_setItemLinks(i); }
...
function image_zoom(on,noresize)
{
	...
	if (!on) {
		image_area.scrollLeft = image_area.scrollTop = 0;
	}
	if (image_zoomon=on) {
		ui_vis('full_size', 0);
		ui_vis('fit_size', 1, 1);
		...
		if (!slide_inprog) {
			...
			if (slide_nextindex() < 0) {
				...
				if (slide_order > 0) {
					...
					t = 'image_show(' + (data_count-1) + ')';
				} else {
					t = 'image_show(' + slide_randomorder[0] + ')';
				}
			}
		}
		slide_inprog = 1;
...
function thumbs_horizvert()
{
	...
	if (thumbs_horiz=thumbs_horiz?0:1) {
		...
		if (app_is_ie) {
			...
			document.getElementById('tools_right').style.paddingRight = '0';
		}
	} else {
		...
		if (app_is_ie) {
			...
			for (o = thumb_div.firstChild; o; o = o.nextSibling) {
				if (o.offsetWidth && o.offsetWidth > i) {
					i = o.offsetWidth;
				}
			}
			...
			thumb_div.style.width = (thumb_div.offsetWidth + 16) + 'px';
		} else if (app_is_safari) {
...
function options_onoff()
{
	if (!options_on) {
		options_setsize();
		if (data_count > 0) {
			options_setItemLinks(image_index);
		}
	}
